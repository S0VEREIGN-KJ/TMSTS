...
if (isset($_POST['condemn_reason'])) {
    $condemn_reason = mysqli_real_escape_string($conn, $_POST['condemn_reason']);
} else {
    $condemn_reason = null; // or leave empty string ''
}
...
if (isset($_POST['release_date'])) {
    ...
    if ($date) {
        $release_date = $date->format('Y-m-d'); // Ensure the correct database format
    } else {
        echo "Invalid release_date format. Expected yyyy-mm-dd.";
        exit;
    }
} else {
...
if ($stmt = $conn->prepare("UPDATE ticket SET 
    ...
    $stmt->bind_param("sssssssssss", $item_date_received, $received_by, $assigned_name, $diagnostic,$priority, $status, $condemn_reason, $comment, $approved_by, $release_date, $ticket_number);
    ...
    header('Location: myticket.php?status=success');
    exit;
} else {
