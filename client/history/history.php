...
// SQL query to retrieve the user's history tickets
$sql = "SELECT * FROM ticket WHERE id_number = ? AND status IN ('Repaired', 'Closed') ORDER BY datetime_req DESC";
...
<div id="feedbackOverlay" class="feedback-overlay">
    <div class="overlay-content">
        ...
        <h2>Feedback</h2>
        <div id="ticketNumberDisplay" class="ticket-number"></div> <!-- Display ticket number here -->
        <form id="feedbackForm" action="submit_feedback.php" method="POST">
            <label for="fullName">Technician Name:</label>
            <input type="text" id="fullName" name="fullName" readonly>

            <input type="hidden" id="ticketId" name="ticketId"> <!-- Hidden input for ticket ID -->

            <label for="feedbackComment">Your Feedback (optional):</label>
            <p> 1 Ticket = 1 Ticket</p>
            This is an Anonymous Feedback
            <textarea id="feedbackComment" name="feedbackComment" required placeholder="Share your thoughts..."></textarea>
...
<div class="table-container">
    <table>
      ...
      while ($row = $result->fetch_assoc()) {
        ...
        echo '<td><a href="#" onclick="openNav(' . $row["ticket_number"] . ')">Details</a></td>'; // Details option click
        echo '<td><a href="#" onclick="openFeedbackOverlay(\'' . $row["ticket_number"] . '\', \'' . $row["assigned_name"] . '\')">Feedback</a></td>'; // Feedback option click
        echo "</tr>";
      }
      ?>
    </table>
...
<div class="overlay_form_group">
  ...
  <input id="priority" name="priority" value="<?php echo $priority; ?>"  readonly> </input> <!-- status-->
          ...
          <div class="overlay_form_group">
              ...
              <input id="status" name="status" value="<?php echo $status; ?> " readonly> </input>   <!-- status-->
